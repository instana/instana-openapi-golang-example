...
func (a *UsageApiService) GetHostsPerDay(ctx _context.Context, day int32, month int32, year int32) ([]UsageResult, *_nethttp.Response, error) {
	...
	localVarPath = strings.Replace(localVarPath, "{"+"day"+"}", _neturl.PathEscape(parameterToString(day, "")), -1)

	localVarPath = strings.Replace(localVarPath, "{"+"month"+"}", _neturl.PathEscape(parameterToString(month, "")), -1)

	localVarPath = strings.Replace(localVarPath, "{"+"year"+"}", _neturl.PathEscape(parameterToString(year, "")), -1)
